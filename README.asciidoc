...
* A deck of 56 cards.
** Cards start in a shuffled event _deck_, and are slowly drafted into players
   _hands_ as they explore.  Players have personal _discard_ piles that
   eventually get recovered into their hands.  A player's discard pile should
   be kept so each previous card's color is visible at all times.  The top card
   of a player's discard pile is _active_, its icon showing when it will take
...
** Resources start in a collective _bank_.  Players gain and spend resources,
   ...
   resources should be visible at all times, except when bidding against
   collapse.
...
. The last player to discover something is the first player. (This player must
  explore.)
...
** This may mean you don't draw any, if there are already at least three empty
   sectors.
. Deal three cards to each player and draft.
.. To draft, choose one card and pass the rest clockwise.
.. You may either take the chosen card into your hand or consume it for
   resources:
*** The first draft pick is worth the highest uncontrolled sector value, the
    second worth the second-highest, the third worth the third-highest.
*** When a card is consumed, it goes into the common dead pile.
...
.. If you cannot pay, destroy cards from your hand until you can.
...
== Endgame
When the last disk is placed, all players recover their discard.  Then continue
taking turns reaping (and collapsing) until one player is out of cards.  At
that point, whoever has the most resources wins.
